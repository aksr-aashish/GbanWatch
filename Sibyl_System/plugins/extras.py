...
async def addenf(event) -> None:
    ...
    if HEROKU:
        ...
        if not event.sender_id in SIBYL:
            await add_enforcers(event.sender_id, u_id)
        await System.disconnect()
        os.execl(sys.executable, sys.executable, *sys.argv)
        quit()
    if event.sender_id not in SIBYL:
        await add_enforcers(event.sender_id, u_id)
...
async def join(event) -> None:
    ...
    if private:
        ...
        await System.send_message(
            Sibyl_logs,
            f"{(await event.get_sender()).first_name} made GbanWatch join {private.group(5)}",
        )
    else:
...
async def leave(event) -> None:
    ...
    if c_id:
        ...
        await System.send_message(event.chat_id, f"GbanWatch has left chat[{link}]")
